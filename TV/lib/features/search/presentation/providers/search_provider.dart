...
final searchDiscoveryProvider = FutureProvider.family<List<SearchChart>, String>((ref, category) async {
  ...
  final results = await Future.wait([
    repo.filter(category: apiCategory, page: 1).timeout(const Duration(seconds: 10)),
    
    isAnime 
      ? repo.filter(category: 'anime', status: 'releasing').timeout(const Duration(seconds: 10))
      : repo.filter(category: apiCategory, year: DateTime.now().year).timeout(const Duration(seconds: 10)),
    
    isAnime
      ? repo.filter(category: 'movie_anime').timeout(const Duration(seconds: 10))
  ...
  if (results[1].results.isNotEmpty) {
    charts.add(SearchChart(
      title: isAnime ? 'Simulcast Activos' : 'Estrenos del Año', 
